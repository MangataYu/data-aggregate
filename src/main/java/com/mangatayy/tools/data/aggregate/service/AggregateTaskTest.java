 ...
 */
@Slf4j
public class AggregateTaskTest extends Thread {
    ...
    public AggregateTaskTest(JdbcTemplate jdbcTemplate, AggregateJobCounterService jobCounterService, AggregateJob job) {
        this.jdbcTemplate = jdbcTemplate;
        this.jobCounterService = jobCounterService;
        this.job = job;
    }

    public AggregateTaskTest(JdbcTemplate jdbcTemplate, AggregateJobCounterService jobCounterService,
    ...
    public void executePreTask(LocalDateTime startTime, LocalDateTime endTime) {
        ...
        // 测试环境制造数据 begin
        PmResultSetExtractorTest handler = new PmResultSetExtractorTest(job.getTarget_table(), jdbcTemplate, kpiCounters, param,
                job.getRemoveNullEnci(), startTime);
        LocalDateTime fixStart = LocalDateTime.of(2022, 8, 25, 0, 0);
        LocalDateTime fixEnd = fixStart.plusMinutes(15);
        jdbcTemplate.query(sql, handler, fixStart, fixEnd);
        // 测试环境制造数据 end
    ...
    private void triggerChild(LocalDateTime startTime) {
        ...
        job.getChildren().forEach(childJob -> {
            ...
            if (isPropertyTime(startTime, childJob.getTriggered_condition())) {
                ...
                new AggregateTaskTest(jdbcTemplate, jobCounterService, childJob,
